<?php function create_shortcode_twyoutube_search($attr) {  ob_start();?>
...
<div class="container">  
	...
	<div class="twyt-search-wrapper flex d-flex-wrap d-gap">
		
	</div>
	
</div>
<input type="hidden" name="twyt_api_key" value="<?php echo $API_Key;?>">
<input type="hidden" name="twyt_search_channel_id" value="<?php echo $channelid ? $channelid: $Channel_mainID;?>">
<input type="hidden" name="twyt_search_max_result" value="<?php echo $maxresult;?>">
<input type="hidden" name="twyt_search_order" value="<?php echo $order;?>">
<input type="hidden" name="twyt_search_q" value="<?php echo $q;?>">
<input type="hidden" name="twyt_search_statics" value="<?php echo $showstatics ? $showstatics: '0';?>">
...
$(document).ready(function() {
	...
	twyt_search_channel_id = $("input[name='twyt_search_channel_id']").val();
	twyt_search_max_result = $("input[name='twyt_search_max_result']").val();
	twyt_search_order = $("input[name='twyt_search_order']").val();
	twyt_search_q = $("input[name='twyt_search_q']").val();
	twyt_search_statics = $("input[name='twyt_search_statics']").val();
	...
	if( twyt_search_q){
		...
		fetch(`https://youtube.googleapis.com/youtube/v3/search?channelId=${twyt_search_channel_id}&order=${twyt_search_order}&q=${twyt_search_q}&maxResults=${twyt_search_max_result}&key=${yt_api_key}`)
			...
			data => {
				...
				for ( itemyt of datasearchYt) {
					...
					.then((data) => {
						...
						}else if(twyt_search_statics == 0){
							twyt_statics_layout = ``;
						}
						$('.twyt-search-wrapper').addClass(`${twyt_layout_desktop} ${twyt_layout_mobile}`)
						$('.twyt-search-wrapper').append(`
...
add_shortcode('twYoutubeApi-search', 'create_shortcode_twyoutube_search');?>